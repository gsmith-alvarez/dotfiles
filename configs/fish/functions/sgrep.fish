...
function sgrep -d "Interactive Ripgrep (dynamic reloading engine)"
    ...
    # Eagerly resolve the editor before dropping into fzf
    set -l fzf_editor (set -q EDITOR; and echo "$EDITOR"; or echo nvim)
    ...
    fzf --ansi --disabled --query "$INITIAL_QUERY" --track --id-nth 1,2 \
