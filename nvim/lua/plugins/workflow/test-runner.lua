...
M.setup = function()
  ...
  local runners = {
    rust = {
      ...
      nearest = function(root, name)
        return 'cd ' .. root .. ' && cargo test ' .. name
      end,
    },
    zig = {
      all = function(root)
        return 'cd ' .. root .. ' && zig build test'
      end,
      file = function(file)
        return 'zig test ' .. file
      end,
      nearest = function(root, _)
        return 'cd ' .. root .. ' && zig build test'
      end,
    },
    python = {
      all = function(root)
        return 'cd ' .. root .. ' && pytest'
      end,
      file = function(file)
        return 'pytest ' .. file
      end,
      nearest = function(_, name)
        return 'pytest -k ' .. name
      end,
    },
    c = {
      all = function(root)
      ...
      file = function(_)
        return nil
      end, -- no per-file runner for C
      nearest = function(_, _)
        return nil
      end,
    },
    cpp = {
  ...
  M.run_all = function()
    ...
    if cmd then
      ...
      vim.notify('No project-wide runner for ' .. key, vim.log.levels.WARN)
    end
  end
  ...
  M.run_file = function()
    ...
    if cmd then
      ...
      vim.notify('No file-level runner for ' .. key, vim.log.levels.WARN)
    end
  end
  ...
  M.run_nearest = function()
    ...
    if cmd then
      ...
      vim.notify('No nearest-test runner for ' .. key, vim.log.levels.WARN)
    end
  end
end
