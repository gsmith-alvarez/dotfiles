...
typst_preview.setup {
  ...
  preview_args = {
    '--port',
    '9527', -- Fixed port to avoid collisions
    '--input',
    'main.typ', -- Common entry point
  },
...
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function(event)
    ...
    vim.keymap.set('n', '<leader>typ', '<cmd>TypstPreview<cr>', { buffer = event.buf, desc = 'Typst: Open [P]review' })
    vim.keymap.set('n', '<leader>tyc', '<cmd>TypstPreviewStop<cr>', { buffer = event.buf, desc = 'Typst: [C]lose Preview' })

    -- "Sync" map: Force the preview to jump to your current cursor position
    vim.keymap.set('n', '<leader>tys', '<cmd>TypstPreviewSync<cr>', { buffer = event.buf, desc = 'Typst: [S]ync Preview' })
