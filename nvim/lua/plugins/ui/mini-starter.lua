...
M.setup = function(force)
  ...
  if not force and vim.fn.argc() ~= 0 then
    return
  end
  is_setup = true
  ...
  local function build_footer()
    ...
    local branch = vim.fn.system('git -C ' .. vim.fn.getcwd() .. ' rev-parse --abbrev-ref HEAD 2>/dev/null'):gsub('%s+$', '')
  ...
  local my_items = {
    vim.fn.isdirectory(paths.projects) == 1 and { name = 'Projects 󰉖 ', action = snacks_action(paths.projects), section = '  Workspaces' } or nil,
    vim.fn.isdirectory(paths.code) == 1 and { name = 'Code  ', action = snacks_action(paths.code), section = '  Workspaces' } or nil,
    vim.fn.isdirectory(paths.dotfiles) == 1 and { name = 'dotfiles 󰄻 ', action = snacks_action(paths.dotfiles), section = '  Workspaces' } or nil,
    vim.fn.isdirectory(paths.obsidian) == 1 and { name = 'Obsidian 󰌱 ', action = snacks_action(paths.obsidian), section = '  Workspaces' } or nil,
