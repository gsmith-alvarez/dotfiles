...
-- ARCHITECTURE SHIFT: We no longer create the group here.
-- We receive the Master Switch from the loader (init.lua) and bind our logic to it.
M.setup = function(injected_group)
	...
	vim.api.nvim_create_autocmd('BufReadCmd', {
		...
		callback = function(args)
			...
			if not ouch then
				utils.soft_notify('ouch-cli missing! Install via: mise install ouch', vim.log.levels
				.WARN)
				return
			end
			...
			if obj.code == 0 then
				...
				utils.soft_notify('ouch failed to decode archive: ' .. (obj.stderr or "Unknown error"),
		...
		callback = function(ev)
			...
			if ok and stats and stats.size > max_filesize then
				...
				if extension == 'json' then
					vim.notify(
						string.format(
						"⚠️ Big JSON Detected (%sMB).\nPerformance degraded. Use :Jless for large datasets.",
							size_mb),
						vim.log.levels.WARN,
					...
					vim.notify(
						string.format(
						"🚀 Big File Mode: Optimized for %sMB file (Syntax/Treesitter OFF).",
							size_mb),
						vim.log.levels.WARN,
						{ title = "Resource Guard" }
					)
