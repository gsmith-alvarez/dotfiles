  ...
  Reload = {
    ...
    impl = function()
      ...
      for module_name, _ in pairs(package.loaded) do
        if module_name:match '^core' or module_name:match '^autocmd' or module_name:match '^commands' or module_name:match '^plugins' then
      ...
      -- 4. CONFIRMATION
      if ok then
        vim.notify('System Reloaded', vim.log.levels.INFO, { title = 'System' })
      else
        vim.notify('System Crash during reload:\n' .. tostring(err), vim.log.levels.ERROR, { title = 'System' })
      end
    end,
  },
}
